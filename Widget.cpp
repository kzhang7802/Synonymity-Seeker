...
void Widget::printResult()
{
    ...
    // If number of synonyms will exceed the limit display, then display as detailed text
    if (bfsRating.size() > DISPLAY_LIMIT)
...
void Widget::dfsRatingAlgorithm(const std::string &originalWord)
{
    ...
    for (auto & i : dfsVect) {
        ...
        rating = 10.0;
        // Find the part of speech of this particular vertex
        std::string partOfSpeech = graph.findVertex(i.first.getName()).getSpeech();
